...
Fabric_Client.newDefaultKeyValueStore({ path: store_path
	...
	var request = {chaincodeId: 'vlm',
		fcn: 'addVehicle',
		args: [req.body.ChasisNumber, req.body.EngineNumber, req.body.VehicleModel, req.body.VehicleMake, req.body.YearOfManufacturing, req.body.Colour,req.body.SeatingCapacity,req.body.VehicleInitialValue],
		chainId: 'vlmchannel',
		txId: tx_id};
	...
	if ( results.status === 'SUCCESS') {
		...
		res.send({code:"500", message: "register failed"});
	}
	...
	res.send({code:"500", message: "Registeration failed"});
});
}

// Issue LC by Bank
function transferVehicle(req, res) {
	...
	Fabric_Client.newDefaultKeyValueStore({ path: store_path
		...
		var request = {chaincodeId: 'vlm',
	fcn: 'transferVehicle',
	args: [req.body.VehicleId,req.body.Owner],
	chainId: 'vlmchannel',
	txId: tx_id};
	...
	res.send({"code":200,"Message":"Success"})
			...
			console.error('Failed to order the transaction. Error code: ' + response.status);
			res.send({code:"500", message: "transfer  failed."});
		}
	...
	Fabric_Client.newDefaultKeyValueStore({ path: store_path
		...
		if (query_responses && query_responses.length == 1) {
			if (query_responses[0] instanceof Error) {
				...
				res.send({code:"500", data: " getting vehicle details went worng"});
			} else {
			...
			res.send({code:"500", data: "No vehicle found"});
		}
	}).catch((err) => {
		console.error('Failed to query successfully :: ' + err);
		res.send({code:"500", data: "error getting vehicle details"});
	});
